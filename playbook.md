...
| --- | --- |
| **mcctl**（必需） | 什么都做不了 |
| AdvancedInfoFetcher | 只能靠截图判断背包、血量和聊天回显 |
...
* `/msg` **读取即清空**：只给上一次请求之后的新消息；`HEAD /msg` 返回 `405`（会白白吃掉回显，所以只允许 `GET`）。
...
* 只记聊天栏；物品栏上方的**动作栏（overlay）提示**（如「方块：被放置」）不在其中，那类反馈仍要截图。
...
④ 校验  GET :3421/msg 看回显 + 再读一次状态对账，必要时 GET :3420/prtsc
   ↺ 回到 ①
```

### ① 观察

* 常规：`/info`（在哪、面朝哪、血够不够）+ `/inventory`（有什么、熔炉烧到哪了）+ `/msg`（上一步的回显）。
...
| `/msg` 里没有「方块：被放置」 | `/msg` 只记聊天栏；动作栏（overlay）提示不在其中，这类反馈要截图 |
