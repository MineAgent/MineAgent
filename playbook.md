...
`wooden_pickaxe` `stone_pickaxe` `iron_pickaxe` `iron_sword` `shield` `bow` `arrow`
`raw_iron` `iron_ingot` `coal` `cobblestone` `bucket` `water_bucket`
`bread` `cooked_beef` `bed` `white_wool`
