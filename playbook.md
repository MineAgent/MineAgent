...
12. **动静读 `GET :3421/sound`**：破坏方块、脚步、怪物、爆炸、UI 的音效 ID 都在里面。
...
* `/msg` 和 `/sound` **读取即清空**：只给上一次请求之后的新内容；`HEAD` 返回 `405`（会白白吃掉内容，所以只允许 `GET`）。
...
* 环境音和脚步很频繁，轮询间隔别太长，否则一次读到很多行；只想看重点就自己 grep 关心的 ID。
...
④ 校验  GET :3421/msg 看回显 + GET :3421/sound 听动静 + 再读一次状态对账，必要时 GET :3420/prtsc
   ↺ 回到 ①
```

### ① 观察

* 常规：`/info`（在哪、面朝哪、血够不够）+ `/inventory`（有什么、熔炉烧到哪了）+ `/msg`（上一步的回显）+ `/sound`（刚才的动静）。
...
* **再读 `/sound`**：挖矿是否真的挖到（`minecraft:block.stone.break`）、附近有没有怪（`minecraft:entity.zombie.*`）、有没有爆炸，听动静比截图快。
...
| 想知道刚才挖到/听到什么 | 读 `GET :3421/sound`，能看到 `minecraft:block.stone.break` 这类音效 ID |
| `/sound` 里一堆脚步/环境音 | 正常：它们本来就很频繁；只 grep 关心的 ID，或缩短轮询间隔 |
